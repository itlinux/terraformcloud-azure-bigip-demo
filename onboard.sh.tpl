...
echo -e "\n"$(date) "Application Services Templates (FAST) Pkg"
curl -L -k -o ${libs_dir}/$FAST_FN $FAST_URL
...
# Check FAST Ready
CNT=0
echo -e "\n"$(date) "Check FAST Ready"
while true
do
  STATUS=$(curl -u $CREDS -X GET -s -k -I https://localhost/mgmt/shared/fast/info| grep HTTP)
  if [[ $STATUS == *"200"* ]]; then
    echo -e "\n"$(date) "Got 200! FAST is Ready!"
    break
  elif [ $CNT -le 6 ]; then
    echo -e "\n"$(date) "Status code: $STATUS  FAST Not done yet..."
    CNT=$[$CNT+1]
  else
    echo -e "\n"$(date) "(FAST) GIVE UP..."
    break

...
resource local_file vm_as3_2_file {
  content  = data.template_file.as3_2_json.rendered
  filename = "${path.module}/mgmt.json"
}
...
resource null_resource AS3_sample {
...
resource null_resource AS3_mgmt {
  depends_on = [null_resource.AS3_sample]
  # Running AS3 REST API
  provisioner "local-exec" {
    command = <<-EOF
      #!/bin/bash
      curl -k -X ${var.rest_as3_method} https://${azurerm_public_ip.vm01mgmtpip.ip_address}${var.rest_as3_uri} -u ${var.uname}:${var.upassword} -d @${path.module}/mgmt.json
    EOF
  }
}
